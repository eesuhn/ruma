...
import { getEditionAcc, getMasterEditionAcc, getMasterOrPrintedEditionPda, getMetadataAcc, getMetadataPda } from '@/lib/umi';
...
export default function Page() {
  ...
  const { data: profileData, isLoading, error } = useSWR(async () => {
    const userPda = getUserPda(publicKey!);
    const userAcc = await getUserAcc(userPda);
    const allEvents = await getAllEventAcc();

    const badges = await Promise.all(userAcc!.badges.map(async (badge) => {
      const { name, uri } = await getMetadataAcc(getMetadataPda(badge));
      const { publicKey: userBadgePda, parent } = await getEditionAcc(getMasterOrPrintedEditionPda(badge));
      const { publicKey: eventBadgePda } = await getMasterEditionAcc(getMasterOrPrintedEditionPda(toWeb3JsPublicKey(parent)));
      const eventBadge = toWeb3JsPublicKey(eventBadgePda);
      const matchedEvent = allEvents.find(({ account }) => account.badge?.equals(eventBadge));
    ...
    allEvents.forEach(async ({ account, publicKey: eventPda }) => {
      if (account.organizer.equals(userPda) && account.data.startTimestamp < Date.now()) {
        eventsHosted++;
      } else {
        const attendeeAcc = await getAttendeeAcc(getAttendeePda(userPda, eventPda));
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <h1 className="text-2xl font-bold">{userAcc.data.name}</h1>
                  </div>
          ...
          {Boolean(badges.length) ? (
            <div>
              ...
              <div className="grid grid-cols-6">
                {badges.map((
                  { pda, name, image, eventName }:
                    { pda: PublicKey, name: string, image: string, eventName: string }) => (
