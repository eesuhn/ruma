...
const sampleNFTs = Array(20).fill('/img/sample-nft.svg?height=150&width=150')
...
export function NftSelection() {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <p className="mb-8 text-muted-foreground">Choose or upload an NFT image for your event attendees.</p>

      <div className="grid md:grid-cols-2 gap-8 mb-8">
        <div>
          <Label htmlFor="nft-upload" className="block mb-2">Upload Custom NFT</Label>
          <Input 
            id="nft-upload" 
            type="file" 
            accept="image/*" 
        ...
        {selectedNFT && (
          <div className='mt-[-120px]'>
            ...
            <img src={selectedNFT} alt="Selected NFT" className="max-w-full h-auto" />
          </div>
        )}
      </div>
      ...
      <ScrollArea className="h-[400px] w-full rounded-md border p-4">
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {sampleNFTs.map((nft, index) => (
            <Card 
              key={index} 
              className={`cursor-pointer transition-all ${selectedNFT === nft ? 'ring-2 ring-primary' : ''}`}
              onClick={() => handleNFTSelect(nft)}
            >
              <CardContent className="p-2">
                <img src={nft} alt={`Sample NFT ${index + 1}`} className="w-full h-auto" />
              </CardContent>
            </Card>
          ))}
        </div>
